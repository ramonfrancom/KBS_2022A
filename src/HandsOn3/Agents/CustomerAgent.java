...
public class CustomerAgent extends Agent {
	...
	private String [] sellersIds = {"amazon1", "amazon2", "alibaba"};
//        private String [] sellersIds = {"amazon1", "amazon2"};
//        private String [] sellersIds = {"amazon1"};
	private int nResponders = sellersIds.length;

	private int nProductos;
        
	protected void setup() {
                // Extract arguments passed to the agent
		Object[] args = getArguments();
		Object[] products = getArguments() != null ? new Object[getArguments().length - 1] : null;
		String payment = ""; 
		String order = "";

                // Generate String of order by the format [payment]:[Array of wanted products]
		if(args != null && args.length > 1){
			payment = args[args.length - 1].toString(); //we get the payment method
                           
			for(int i = 0; i < products.length; i++){
				products[i] = args[i];
			}

			order = payment + ":" + Arrays.toString(products);
		}
                
                // Send order to the sellers
		if (products != null && products.length > 0) {
			System.out.println(getLocalName() + " is sending orders.");
			
			// Fill the CFP message
			ACLMessage msg = new ACLMessage(ACLMessage.CFP);
			
			for (String seller :sellersIds) { //adding the recivers
				msg.addReceiver(new AID(seller, AID.ISLOCALNAME));
			}

			msg.setProtocol(FIPANames.InteractionProtocol.FIPA_CONTRACT_NET);
			msg.setReplyByDate(new Date(System.currentTimeMillis() + 15000)); // We want to receive a reply in 15 secs, otherwise we call 
			...
			addBehaviour(new ContractNetInitiator(this, msg) {
				...
				protected void handlePropose(ACLMessage propose, Vector v) {
					System.out.println("Seller " + propose.getSender().getLocalName() + " has some of the products ");
				}
				...
				protected void handleAllResponses(Vector responses, Vector acceptances) {
					...
					if (responses.size() < nResponders) {
						...
						System.out.println("Timeout expired: missing "+(nResponders - responses.size())+" responses");
                                        ...
                                        // Put in a list all the sellers proposes
					Enumeration e = responses.elements();
					while(e.hasMoreElements()) {
						ACLMessage msg = (ACLMessage) e.nextElement();
						if (msg.getPerformative() == ACLMessage.PROPOSE)
							for (Offer offer :getReponderOffers(msg)){ //we get the offer for each product sold by a seller
								offers.add(offer);
							}
					}
					...
					for(String seller :sellersIds){ //get the seller offer to send them an accept proposal
				...
				protected void handleInform(ACLMessage inform) {
					System.out.println("Agent "+inform.getSender().getLocalName()+ " says: " + inform.getContent());
				}
			} );
		}
		else {
			System.out.println("No products or unspecified payment method.");
		}
  	} 
	...
	public class Offer {
		...
		public String toString(){
			return this.name + " for $" + String.valueOf(price) + " dollars";
		}
